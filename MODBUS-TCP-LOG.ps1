...
for ( $i = 0; $i -lt $args.count; $i++ ) {
    ...
    if ($args[ $i ] -eq "-a") { 
        $MODBUS_address = MinMaxSetting 0 254 $args[ $i + 1 ]
        Write-Debug("MODBUS address = $MODBUS_address")
    }
    if ($args[ $i ] -eq "-l") { 
...
if ($GetMACAddress) {
    ...
    if ($test_ping) {
        ...
        $read_mac_address = Read-HoldingRegisters -Address $computername -Port $portnumber -Reference 517 -Num 6 -Slave $MODBUS_address
...
do {
    ...
    if ($second -eq ($nextsamplesec)) {
        ...
        if ($test_ping) {
            ...
            $one_channel = Read-HoldingRegisters -Address $computername -Port $portnumber -Reference 0 -Num 8 -Slave $MODBUS_address
